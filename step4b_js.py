...
# Make sure to not duplicate if run multiple times
if "cargarEstudiantesClean" not in content:
    # Reverse string search to replace only the last </script>
    parts = content.rsplit('</script>', 1)
    if len(parts) == 2:
        content = parts[0] + js_functions + "\n</script>" + parts[1]

    with open('VistaProfesor.html', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Injected JS perfectly.")
else:
